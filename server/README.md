...
| POST | `/v1/auth/password` | 改密：`{old_password,new_password,revision,envelope?}`。有云端密文时必须带用新密码重封的信封；成功后作废全部 refresh，并立即使旧 access token 失效 |
| GET | `/v1/me` | 当前用户 |
| DELETE | `/v1/me` | `{password}` 注销账号并删除云端密文 |
| GET | `/v1/sync` | 取加密信封 + revision |
| PUT | `/v1/sync` | `{revision, envelope}`，revision 不对返回 409 |

密码至少 8 位，邮箱小写去重，无需验证。Access token 默认 15 分钟，refresh 30 天；过期或已作废的 refresh 会在登录 / 刷新时清掉。改密会把用户的 token 版本加一，旧 access token 马上不能再调用接口。
