...
struct ReportRootView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            .frame(minHeight: 140, idealHeight: 240, maxHeight: 280)
            .layoutPriority(-1)
            models
            details
        }
    ...
    var filters: some View {
        ChipFlowLayout(hSpacing: 10, vSpacing: 8) {
    ...
    var kpiStrip: some View {
        ...
        return HStack(alignment: .top, spacing: 32) {
            kpiCell("请求", "\(report.eventCount)")
            kpiCell("Token", UsageParser.formatTokenCount(Double(report.totalTokens)))
            if report.hasCost { kpiCell("费用", UsageParser.formatUSDCents(report.totalCents)) }
            if discount != "—" { kpiCell("折扣", discount) }
            let perMillion = StatusText.formatReportPerMillionKpi(totalCny: report.totalCny, totalTokens: report.totalTokens)
            if !perMillion.isEmpty { kpiCell("每百万Token", perMillion) }
            if let usage = store.app.usage, usage.showsAmount {
                kpiCell("企业额度（展示）", UsageParser.formatSpendRange(used: usage.usedCents, limit: usage.limitCents))
            }
        }
    }

    var mixBlock: some View {
        Text(mixText)
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    func kpiCell(_ label: String, _ value: String) -> some View {
    ...
    var mixText: String {
        ...
        mix += StatusText.formatReportSpendKpi(
    ...
    var models: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Table(store.report.models) {
                ...
                TableColumn("Token") { row in Text(UsageParser.formatTokenCount(Double(row.tokens))).frame(maxWidth: .infinity, alignment: .trailing) }
                TableColumn("费用") { row in Text(row.cents > 0 ? UsageParser.formatUSDCents(row.cents) : "—").frame(maxWidth: .infinity, alignment: .trailing) }
                TableColumn("实付") { row in Text(row.cny > 0 ? UsageEvents.formatCNY(row.cny) : "—").frame(maxWidth: .infinity, alignment: .trailing) }
                TableColumn("折扣") { row in
                    Text(UsageEvents.formatDiscount(cny: row.cny, cents: row.cents, rate: store.report.usdCnyRate))
                        .frame(maxWidth: .infinity, alignment: .trailing)
                }
                TableColumn("次数") { row in Text(String(row.count)).frame(maxWidth: .infinity, alignment: .trailing) }
                TableColumn("云端") { row in Text(row.headlessCount > 0 ? String(row.headlessCount) : "—").frame(maxWidth: .infinity, alignment: .trailing) }
            }
    ...
    var details: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Table(store.report.events) {
                ...
                TableColumn("Token") { ev in Text(UsageParser.formatTokenCount(Double(ev.tokens))).frame(maxWidth: .infinity, alignment: .trailing) }
                TableColumn("费用") { ev in Text(UsageEvents.formatCost(ev)).frame(maxWidth: .infinity, alignment: .trailing) }
                TableColumn("实付") { ev in Text(UsageEvents.formatEventCny(ev)).frame(maxWidth: .infinity, alignment: .trailing) }
