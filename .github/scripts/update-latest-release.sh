...

{
  ...
  printf '%s\n' ""
  printf '%s\n' "Windows 提供安装版 [\`CursorRemain-windows-setup.exe\`](https://github.com/${GITHUB_REPOSITORY}/releases/download/latest/CursorRemain-windows-setup.exe) 与 zip 便携包（均为 .NET 8 框架依赖，需本机已装 [Desktop Runtime](https://aka.ms/dotnet/8.0/windowsdesktop-runtime-win-x64.exe)），macOS 为 Swift \`.app\`。"
